//! House-style TOML formatter — wraps the taplo crate with a fixed style so a config file stays
...
/// Format TOML in the shared house style: nested-table indentation, aligned `=` and trailing
/// comments, authored key order preserved, and blank lines separating top-level/group sections
/// while nested tabs stay tight (taplo handles all but the blank-line normalization — see
/// `separate_sections`).
...
/// the missing separators and strips blanks that crept in before a tab.
///
/// "Tight vs separated" keys on the header's leaf segment — a leaf of `tab` (the per-tab entry
/// both apps use) is tight; anything else is a container, separated. A comment block glued to a
/// header (no blank between them) is treated as part of that section, so the inserted/stripped
/// blank applies above the comment, never between the comment and its header. No blank is added
/// at the very start of the file. Idempotent: a file already spaced this way is returned
/// unchanged.
...
/// Format `path` in place. Reads, formats, and rewrites **only if the bytes change** (so a
/// watcher driving this can't loop — an already-formatted file is a no-op). The rewrite is
/// atomic (temp file + rename) and **identity-preserving**: it resolves symlinks so a linked
/// config (e.g. one symlinked in from a dotfiles repo) is rewritten in place rather than
/// replaced by a regular file, and it copies the original's permissions onto the replacement.
